...
def compute_stats(checkpoint_path):
    ...
    #   <dir>/pretrain/<task>/eval_log.json  (old, with split subdirs)
    #   <dir>/<task>/eval_log.json           (new, flat)
    has_split_dirs = any(
        os.path.isdir(os.path.join(checkpoint_path, s))
        for s in ["pretrain", "target"]
    )
    ...
    if has_split_dirs:
        ...
        # Flat layout: scan root directory for <task>/eval_log.json
        for task_name in sorted(os.listdir(checkpoint_path)):
            task_dir = os.path.join(checkpoint_path, task_name)
            ...
            sr_key = f"success_rate/{task_name}"
            if sr_key in this_data:
                stats["pretrain"][task_name] = int(this_data[sr_key] * 100)
